...
from pcdet.utils.metric import get_hist , get_miou
...
def eval_one_epoch(cfg, args, model, dataloader, epoch_id, logger, dist_test=False, result_dir=None):
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        with torch.no_grad():
            #====model==pillarseg======#
            seg_dicts, label_dicts, pred_dicts, ret_dict = model(batch_dict)
            #====detection model=======#
            # pred_dicts, ret_dict = model(batch_dict)
        if cfg.MODEL['NAME'] =='PillarSeg':
            hist = get_hist(hist, seg_dicts, label_dicts, 4)
            disp_dict = {}
            if getattr(args, 'infer_time', False):
    ...
    if cfg.MODEL['NAME'] !='PillarSeg':
        ...
        result_str, result_dict,results = dataset.evaluation(
        ...
        logger.info('****************Evaluation vod metric.*****************')
        logger.info("Results: \n"
        ...
        # print(f'miou={miou}')
        logger.info('****************Evaluation seg done.*****************')
