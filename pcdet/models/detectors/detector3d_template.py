...
from .. import backbones_2d, backbones_3d, dense_heads, roi_heads , seg_model_2d , feature_fusion
...
class Detector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.module_topology = [
            'vfe', 'backbone_3d', 'map_to_bev_module', 'pfe', 'seg','feature_fusion',
    ...
    def build_seg(self,model_info_dict):
        if self.model_cfg.get('SEG',None) is None:
            return None, model_info_dict

        unet_module = seg_model_2d.__all__[self.model_cfg.SEG.NAME](
            n_channels=self.model_cfg.SEG.N_CHANNELS,
            n_classes=self.model_cfg.SEG.N_CLASSES,
            bilinear=self.model_cfg.SEG.BILINEAR,
        )

        model_info_dict['module_list'].append(unet_module)

        return unet_module, model_info_dict

    def build_feature_fusion(self,model_info_dict):
        if self.model_cfg.get('FEATURE_FUSION',None) is None:
            return None, model_info_dict

        fusion_module = feature_fusion.__all__[self.model_cfg.FEATURE_FUSION.NAME](
            in_channels=self.model_cfg.FEATURE_FUSION.IN_CHANNELS,
            out_channels=self.model_cfg.FEATURE_FUSION.OUT_CHANNELS,
            use_attn = self.model_cfg.FEATURE_FUSION.USE_ATTN,
            use_reference_pts=self.model_cfg.FEATURE_FUSION.USE_EMPTY_VOXEL,
        )
