...
class Radar7PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range):
        ...
        self.use_attention = self.model_cfg.USE_ATTENTION
        self.selected_indexes = []
        self.use_empty_voxel = self.model_cfg.USE_EMPTY_VOXEL
        ...
        if num_filters[0] == 13 and self.use_attention == True:
            self.xyz_layer = nn.Linear(9, 32, bias=False)
            self.doppler_layer = nn.Linear(4, 32, bias=False)
            ...
            for i in range(len(num_filters) - 1):
                in_filters = 14
                out_filters = num_filters[i + 1]
                empty_pfn_layers.append(
                    PFNLayer(in_filters, out_filters, self.use_norm, last_layer=(i >= len(num_filters) - 2))
                )
            self.empty_pfn_layers = nn.ModuleList(empty_pfn_layers)
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        voxel_features, voxel_num_points, coords = batch_dict['voxels'], batch_dict['voxel_num_points'], batch_dict[
            'voxel_coords']
        # voxels_empty, num_points_empty, coordinates_empty = batch_dict['voxels_empty'], batch_dict['num_points_empty'], batch_dict['coordinates_empty']
        ...
        # orig_empty_xyz = voxels_empty[:, :, :self.z_ind + 1]
        ...
        # empty_points_mean = orig_empty_xyz.sum(dim=1, keepdim=True) / num_points_empty.type_as(voxels_empty).view(-1, 1, 1)
        # empty_f_cluster = orig_empty_xyz - empty_points_mean  # offset from cluster mean
        ...
        # empty_f_center = torch.zeros_like(orig_empty_xyz)
        ...
        # empty_f_center[:, :, 0] = voxels_empty[:, :, self.x_ind] - (
        #             coordinates_empty[:, 3].to(voxels_empty.dtype).unsqueeze(1) * self.voxel_x + self.x_offset)
        # empty_f_center[:, :, 1] = voxels_empty[:, :, self.y_ind] - (
        #             coordinates_empty[:, 2].to(voxels_empty.dtype).unsqueeze(1) * self.voxel_y + self.y_offset)
        # empty_f_center[:, :, 2] = voxels_empty[:, :, self.z_ind] - (
        #             coordinates_empty[:, 1].to(voxels_empty.dtype).unsqueeze(1) * self.voxel_z + self.z_offset)

        voxel_features = voxel_features[:, :, self.selected_indexes]  # filtering for used features
        # empty_voxel_features = voxels_empty[:, :, :8]  # filtering for used features

        features = [voxel_features, f_cluster, f_center]
        # empty_features = [empty_voxel_features, empty_f_cluster, empty_f_center]
        ...
        features = torch.cat(features, dim=-1)
        # empty_features = torch.cat(empty_features, dim=-1)
        ...
        # empty_voxel_count = empty_features.shape[1]
        # mask = self.get_paddings_indicator(num_points_empty, empty_voxel_count, axis=0)
        # mask = torch.unsqueeze(mask, -1).type_as(empty_voxel_features)
        # empty_features *= mask

        if self.use_empty_voxel:
            voxels_empty, num_points_empty, coordinates_empty = batch_dict['voxels_empty'], batch_dict['num_points_empty'], batch_dict['coordinates_empty']
            orig_empty_xyz = voxels_empty[:, :, :self.z_ind + 1]
            empty_points_mean = orig_empty_xyz.sum(dim=1, keepdim=True) / num_points_empty.type_as(voxels_empty).view(-1, 1, 1)
            empty_f_cluster = orig_empty_xyz - empty_points_mean
            empty_f_center = torch.zeros_like(orig_empty_xyz)
            empty_f_center[:, :, 0] = voxels_empty[:, :, self.x_ind] - (
                    coordinates_empty[:, 3].to(voxels_empty.dtype).unsqueeze(1) * self.voxel_x + self.x_offset)
            empty_f_center[:, :, 1] = voxels_empty[:, :, self.y_ind] - (
            ...
            empty_f_center[:, :, 2] = voxels_empty[:, :, self.z_ind] - (
                    coordinates_empty[:, 1].to(voxels_empty.dtype).unsqueeze(1) * self.voxel_z + self.z_offset)
            empty_voxel_features = voxels_empty[:, :, :8]
            empty_features = [empty_voxel_features, empty_f_cluster, empty_f_center]
            empty_features = torch.cat(empty_features, dim=-1)
            empty_voxel_count = empty_features.shape[1]
            mask = self.get_paddings_indicator(num_points_empty, empty_voxel_count, axis=0)
        ...
        if self.use_attention == True:
            xyz_feature = torch.cat([features[:,:,:3],features[:,:,7:]],dim=-1)
            doppler_feature = features[:,:,3:7]
            xyz_feature = self.xyz_layer(xyz_feature)
            doppler_feature = self.doppler_layer(doppler_feature)
            x = torch.cat([xyz_feature,doppler_feature],dim=-1)
            torch.backends.cudnn.enabled = False
            x = self.norm(x.permute(0, 2, 1)).permute(0, 2, 1) if self.use_norm else x
            torch.backends.cudnn.enabled = True
            x = F.relu(x)
            features = torch.max(x, dim=1, keepdim=True)[0]
        else:
            for pfn in self.pfn_layers:
                features = pfn(features)
            if self.use_empty_voxel:
                for empty_pfn in self.empty_pfn_layers:
                    empty_features = empty_pfn(empty_features)
        features = features.squeeze()
        # empty_features = empty_features.squeeze()
        batch_dict['pillar_features'] = features
        if self.use_empty_voxel:
            empty_features = empty_features.squeeze()
            batch_dict['empty_pillar_features'] = empty_features
        # batch_dict['empty_pillar_features'] = empty_features
        return batch_dict
